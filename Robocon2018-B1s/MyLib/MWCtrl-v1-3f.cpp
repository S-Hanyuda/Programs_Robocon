...
void MWCtrl::PwmCtrl(int N) { //モーター回転制御関数
	if( (*_Digi[N] != Digital[N]) && (PwmValue[N][0] > 0.0) ) { //まだ逆に回ってる場合
		...
		*_Digi[N] = Digital[N]; //モーター回転方向設定
		...
		PwmSUM[N] = Distance * AngleProp * (1.0 - TurnValue[N]) * Offset[N]; //PwmSUM算出処理
...
void MWCtrl::TurnCalc(unsigned char L2, unsigned char R2, int mode) {
	int m, a;
	m = mode % 2; //1 or 0
	a = TurnAct(L2, R2); //1 or 2 or 0
	if(mode > 2) { //左右
		if(a != 0) {
			if( (m==1 && a==1) || (m==0 && a==2) ) {
